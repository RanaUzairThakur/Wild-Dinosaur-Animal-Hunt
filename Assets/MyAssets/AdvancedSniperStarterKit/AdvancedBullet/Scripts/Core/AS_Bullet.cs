...
public class AS_Bullet : MonoBehaviour
{
    ...
    public void  Start ()
	{
      ...
      //GetComponent<Rigidbody>().constraints = ~RigidbodyConstraints.FreezeAll; GetComponent<Rigidbody>().useGravity = true;

        //AS_ActionCamera.instance.SlowmotionNow(0.002f,10f);
	...
	bool RayPrediction (Vector3 lastpos, Vector3 currentpos, Vector3 initialPosition, float delta)
	{
		...
		for (var i = 0; i < hits.Length; i++) {
			...
			if (bulletHit) {
				if (actionPreset && !firsthited) {
                    Debug.Log("Bullet Script First bool detected :");
    ...
    {
        Gun.instance.followCam.transform.parent = null;
        Gun.instance.CamReset();
        Gun.instance.test = false;
        follow.followChk = false;
        Gun.instance.tempChk = false;
    }
