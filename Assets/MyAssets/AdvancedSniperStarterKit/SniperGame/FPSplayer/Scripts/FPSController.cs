...
public class FPSController : MonoBehaviour
{
	...
	{
       // print("VISIABLE "+visible);
		//if (GunView) {
            //GunView.GetComponent<Camera>().enabled = visible;
            if (visible == true)
            {
                GunView.gameObject.GetComponent<Animator>().SetBool("zoom", false);
                GunView.GetComponent<Camera>().enabled = visible;
            }
            else 
            {
                  GunView.gameObject.GetComponent<Animator>().SetBool("zoom", true);
               // Debug.Log("zooming On");
            if(GunView.GetComponent<Camera>().enabled != false)
            Invoke("gunViewZoom", 0.3f);
    ...
    public void gunViewZoom()
    {
        GunView.GetComponent<Camera>().enabled = false;
        //Debug.Log("zooming On off");

        if (Gun.instance.Zooming == true)
            Gun.instance.crossHairOwnUse.gameObject.SetActive(true);
        else
            Gun.instance.crossHairOwnUse.gameObject.SetActive(false);
